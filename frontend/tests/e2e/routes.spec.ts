...
test("favorites are saved across reloads", async ({ page }) => {
  ...
  await expect(favoriteCard.locator("[data-market-result='spread']")).toHaveText("Final: HOME won by 4");
  await expect(favoriteCard.locator("[data-market-result='total']")).toHaveText("Final: 44");
...
test("graded outcomes override lock styling on every current-pick card type", async ({ page }) => {
  ...
  await expect(game.locator("[data-market-result='spread']")).toHaveText("Final: HOME won by 4");
  ...
  await expect(game.locator("[data-market-result='total']")).toHaveText("Final: 44");
  ...
  await expect(page.locator("[data-lock-market='spread'] [data-market-result='spread']")).toHaveText("Final: HOME won by 4");
