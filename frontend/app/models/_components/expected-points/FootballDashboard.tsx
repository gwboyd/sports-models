    ...
    <article data-game-id={gameDomId(game.game_id)} className={`scroll-mt-36 rounded-lg border bg-white transition-all ${highlighted ? "border-[var(--accent)] ring-2 ring-[var(--accent-soft)]" : "border-[var(--border)]"}`}>
      <details className="group">
        ...
        <div className="border-t border-[var(--border)] px-3 pb-3 pt-2.5">
          ...
          <div className="grid gap-4 sm:grid-cols-2"><MarketDetails game={game} result={result} market="spread" /><MarketDetails game={game} result={result} market="total" /></div>
        </div>
      </details>
    </article>
  );
...
function DesktopMarketCell({ game, result, market }: { game: ExpectedPointsPick; result?: GameResult; market: FootballMarket }) {
