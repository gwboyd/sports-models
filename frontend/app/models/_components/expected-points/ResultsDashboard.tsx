...
function StatCard({ label, record, hint }: { label: string; record?: RecordSummary; hint?: string }) {
  return (
    <article className="rounded-2xl border border-[var(--border)] bg-white p-4 shadow-[0_1px_2px_rgba(16,24,40,0.04)]">
      <p className="text-xs font-bold uppercase tracking-wider text-[var(--muted)]">{label}</p>
      <p className="numbers-tabular mt-3 text-2xl font-bold tracking-tight text-[var(--ink)]">{record ? recordLabel(record) : hint}</p>
    ...
    <main className="mx-auto w-full max-w-7xl px-4 py-6 pb-20 sm:px-6 sm:py-8 lg:px-8">
      <header className="flex flex-col gap-5 border-b border-[var(--border)] pb-6 sm:flex-row sm:items-end sm:justify-between">
        ...
        <label className="text-sm font-medium text-[var(--muted)]">Season
          <select aria-label="Season" value={season} onChange={(event) => changeSeason(event.target.value)} className="ml-2 min-h-11 rounded-xl border border-[var(--border)] bg-white px-3 font-semibold text-[var(--ink)]">
      ...
      <section className="mt-8" aria-labelledby="season-summary"><h2 id="season-summary" className="text-xl font-semibold tracking-tight text-[var(--ink)]">{season === "all" ? "All-time summary" : `${season} summary`}</h2>
      ...
      <section className="mt-10" aria-labelledby="weekly-breakdown"><div><h2 id="weekly-breakdown" className="text-xl font-semibold tracking-tight text-[var(--ink)]">Weekly breakdown</h2><p className="mt-1 text-sm text-[var(--muted)]">Records are wins–losses–pushes. Win rate excludes pushes.</p></div>
        <div className="mt-4 space-y-3 md:hidden">
          {weeks.map((item) => <article key={`${item.season}-${item.week}`} className="rounded-2xl border border-[var(--border)] bg-white p-4"><div className="mb-4 flex items-center justify-between"><strong>Week {item.week}</strong>{season === "all" ? <span className="text-xs font-semibold text-[var(--muted)]">{item.season}</span> : null}</div><div className="grid grid-cols-2 gap-4"><WeekRecord title="Spread" record={item.summary.spread} locks={item.summary.spreadLocks} /><WeekRecord title="Total" record={item.summary.total} locks={item.summary.totalLocks} /></div></article>)}
        </div>
        <div className="mt-4 hidden overflow-hidden rounded-2xl border border-[var(--border)] bg-white md:block">
