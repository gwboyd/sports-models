...
export function OverlayPanel({ title, description, onClose, children }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/35 p-0 sm:items-center sm:p-6" role="presentation" onMouseDown={(event) => {
      ...
      <section role="dialog" aria-modal="true" aria-labelledby="overlay-title" className="max-h-[88dvh] w-full overflow-hidden rounded-t-lg bg-white shadow-xl sm:max-w-xl sm:rounded-lg">
        <header className="flex items-start justify-between gap-4 border-b border-[var(--border)] px-4 py-3.5">
          <div>
        ...
        <div className="max-h-[calc(88dvh-72px)] overflow-y-auto p-4">{children}</div>
      </section>
    </div>
  );
}
